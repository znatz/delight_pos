...
require_once './mapping/receipt_top_class.php';
...
$errorMessage = "";
$successMessage = "";
$targetReceiptTop = Receipt_top::get_one_empty_receipt_top();;
...
$contents = Receipt_top::get_all_receipt_top();


// リスト内ラジオボタンの処理
if (isset($_POST["targetID"])) {
    $chrID = $_POST["targetID"];
    $targetReceiptTop = Receipt_top::get_one_receipt_top($chrID);
...
for ($i = 0; $i <= 99; $i ++) {
    if (isset($_POST[$i])) {
        if (Receipt_top::delete_one_receipt_top($i)) {
            $contents = Receipt_top::get_all_receipt_top();
...
if (isset($_POST["submit"])) {
    $errorMessage = "";
    $successMessage = "";
   if(Receipt_top::insert_one_receipt_top(
       $_POST['chrShop_ID'],
       $_POST['intLine_Count'],
       $_POST['chrComment']))
   {
        ...
        // 更新処理開始
        if  (mysql_errno() == 1062) {
            if(Receipt_top::update_one_receipt_top(
                $_POST['chrShop_ID'],
                $_POST['intLine_Count'],
                $_POST['chrComment']))
            {
    ...
    <div class="main">
            <?php include('./html_parts/warning.html');?>
					...
					<form class="search_form" style="margin: 0 auto; width: 700px; height:220px;"
                        ...
                        <fieldset><legend>レシート上部マスタ</legend></fieldset>
							...
							<p class="list" style="margin-top: 30px;">
                                ...
                                <select
                                    ...
                                    <?php foreach($shops as $c) : ?>
                                        <option value="<?php echo $c[0] ?>" <? if($targetReceiptTop->chrShop_ID==$c[0]) echo "selected"; ?>><?php echo $c[0]." ".$c[1]; ?></option>
                                    <?php endforeach; ?>
                                </select>
							</p>
							<p class="list">
								<label class="list">行No.</label><input
									...
									data-prompt-position="topLeft:140" name="intLine_Count"
									value="<?php
        echo $targetReceiptTop->intLine_Count;
        ?>" />
							</p>
							<p class="list">
								<label class="list">コメント</label> <input
									...
									data-prompt-position="topLeft:140" name="chrComment"
									value="<?php
        echo $targetReceiptTop->chrComment;
        ?>" />
							</p>
							...
							<p style="float: left; text-align: center; width: 300px; margin-top:70px;"
								...
								<input class="center_button hvr-fade" type="submit" name="submit"
									size="10" value="登録"/>
                                <a class="center_button hvr-fade" href="./receipt_top.php" style="display: block; float:left;text-decoration: none; width: 88px; margin: 30px 5px; font-size: 14px; padding: 12px 0 12px 0;">クリア</a>
...
foreach((array)$contents as $c) {
   $chrID_removed[] = new Receipt_top_id_removed($c->chrShop_ID, $c->intLine_Count, $c->chrComment) ;
}
    ...
    <!-- ********************  入力規則　開始      *********************** -->
                        <div id="sidr-right">
                            <?php
                                    ...
                                    $query = 'SELECT txtInstruction FROM form_helper WHERE chrPageName="receipt_top";';
