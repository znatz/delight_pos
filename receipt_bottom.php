...
require_once './mapping/receipt_bottom_class.php';
...
$errorMessage = "";
$successMessage = "";
$targetReceiptTop = Receipt_bottom::get_one_empty_receipt_bottom();;
...
$contents = Receipt_bottom::get_all_receipt_bottom();


// リスト内ラジオボタンの処理
if (isset($_POST["targetID"])) {
    $chrID = $_POST["targetID"];
    $targetReceiptTop = Receipt_bottom::get_one_receipt_bottom($chrID);
...
for ($i = 0; $i <= 99; $i ++) {
    if (isset($_POST[$i])) {
        if (Receipt_bottom::delete_one_receipt_bottom($i)) {
            $contents = Receipt_bottom::get_all_receipt_bottom();
...
if (isset($_POST["submit"])) {
    $errorMessage = "";
    $successMessage = "";
   if(Receipt_bottom::insert_one_receipt_bottom(
       $_POST['chrShop_ID'],
       $_POST['intLine_Count'],
       $_POST['chrComment']))
   {
        ...
        // 更新処理開始
        if  (mysql_errno() == 1062) {
            if(Receipt_bottom::update_one_receipt_bottom(
                $_POST['chrShop_ID'],
                $_POST['intLine_Count'],
                $_POST['chrComment']))
            {
    ...
    <div class="main">
            <?php include('./html_parts/warning.html');?>
					...
					<form class="search_form" style="margin: 0 auto; width: 700px; height:220px;"
                        ...
                        <fieldset><legend>レシート下部マスタ</legend></fieldset>
        ...
        ?>" />
							...
							<p style="float: left; text-align: center; width: 300px; margin-top:70px;"
								...
								<input class="center_button hvr-fade" type="submit" name="submit"
									size="10" value="登録"/>
                                <a class="center_button hvr-fade" href="./receipt_bottom.php" style="display: block; float:left;text-decoration: none; width: 88px; margin: 30px 5px; font-size: 14px; padding: 12px 0 12px 0;">クリア</a>
...
foreach((array)$contents as $c) {
   $chrID_removed[] = new Receipt_bottom_id_removed($c->chrShop_ID, $c->intLine_Count, $c->chrComment) ;
}
    ...
    <!-- ********************  入力規則　開始      *********************** -->
                        <div id="sidr-right">
                            <?php
                                    ...
                                    $query = 'SELECT txtInstruction FROM form_helper WHERE chrPageName="receipt_bottom";';
