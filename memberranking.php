...
$errorMessage = "";
$successMessage = "";
$targetMemberranking = new Category();
...
$contents = Memberranking::get_all();

// 新規ボタンの処理
if (isset($_POST['newID'])) {
    $targetMemberranking = new Memberranking;
    unset($_POST['newID']);
}

// リスト内ラジオボタンの処理
if (isset($_POST["targetID"])) {
    $targetMemberranking = Memberranking::find($_POST["targetID"]);
    unset($_POST["target"]);
    $contents = Memberranking::get_all();
}
...
if (isset($_POST['delete'])) {
    if (Memberranking::delete($_POST['delete'])) {
        $contents = Memberranking::get_all();
...
if (isset($_POST["submit"])) {
    if (Memberranking::insert_values([$_POST['chrID'],
        $_POST['chrName']])
    ) {
        $successMessage = "追加しました。";
    } else {
        if (Memberranking::update_to_column('chrName',$_POST['chrName'],'chrID',$_POST['chrID']))
        {
                ...
                <form class="search_form" style="margin: 0 auto; width: 700px; height: 220px;"
                    ...
                    <p class="list" style="margin:50px 0 0 0;">
                        ...
                        <input
                            ...
                            style="width: 390px; margin: 0 0 0 18px;" name="chrID"
                            type="text" size="10" placeholder="00"
                            value='<?php
                            echo $targetMemberranking->chrID;
                            ?>'/>
                    ...
                    <p class="list">
                        <label class="list">分類</label><input
                            ...
                            value="<?php
                            echo $targetMemberranking->chrName;
                            ?>"/>
                    </p>
                    ...
                    <p style="float: left; text-align: center; width: 300px;margin-top: 80px;"
                       id="buttonlist">
                        <input class="center_button hvr-fade" type="submit" name="submit"
                               size="10" value="登録"/>
                        <a class="center_button hvr-fade" href="./memberranking.php" style="display: block; float:left;text-decoration: none; width: 88px; margin: 30px 5px; font-size: 14px; padding: 12px 0 12px 0;">クリア</a>
...
<div id="sidr-right">
    ...
    $query = 'SELECT txtInstruction FROM form_helper WHERE chrPageName="memberranking";';
