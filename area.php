...
if (isset($_POST["targetID"])) {
    $targetArea = Area::find($_POST["targetID"]);
    unset($_POST["target"]);
    $contents = Area::get_all();
}
...
if (isset($_POST["submit"])) {
    if (Area::insert_values([$_POST['chrID'],
        $_POST['chrName']])
    ) {
        $successMessage = "追加しました。";
    } else {
        if (Area::update_to_column('chrName',$_POST['chrName'],'chrID',$_POST['chrID']))
        {
...
<!--                        <input class="newID hvr-fade"
 ...
 -->
                    ...
                    <p class="list">
                        <label class="list">地区名</label><input
                    ...
                    <p style="float: left; text-align: center; width: 300px;margin-top: 80px;"
                       id="buttonlist">
                        <input class="center_button hvr-fade" type="submit" name="submit"
                               size="10" value="登録"/>
                        <a class="center_button hvr-fade" href="./area.php" style="display: block; float:left;text-decoration: none; width: 88px; margin: 30px 5px; font-size: 14px; padding: 12px 0 12px 0;">クリア</a>
...
<div id="sidr-right">
    ...
    $query = 'SELECT txtInstruction FROM form_helper WHERE chrPageName="area";';
