...
require_once './mapping/telop_class.php';
...
$errorMessage = "";
$successMessage = "";
$targetTelop = Telop::get_one_empty_telop();;
...
$contents = Telop::get_all_telop();

// リスト内ラジオボタンの処理
if (isset($_POST["targetID"])) {
    $chrShop_ID = $_POST["targetID"];
    $targetTelop = Telop::get_one_telop($chrShop_ID);
    ...
    <div class="main">
            <?php include('./html_parts/warning.html');?>
					...
					<form class="search_form" style="margin: 0 auto; width: 700px; height:220px;"
                        ...
                        <fieldset><legend>テロップマスタ</legend></fieldset>
                        <a class="center_button hvr-fade" style="margin:0 auto;float:right;width:10px;" id="right-menu" href="#sidr">入力説明表示／非表示 <i class="fa fa-info-circle"></i></a>
							<p class="list" style="margin-top: 50px;">
                                ...
                                <select name="chrShop_ID" style="float:left;height:33px;width:207px;">
                                    <?php foreach($shops as $c) : ?>
                                        <option value="<?php echo $c[0] ?>" <? if($targetTelop->chrShop_ID==$c[0]) echo "selected"; ?>><?php echo $c[0]." ".$c[1]; ?></option>
                                    <?php endforeach; ?>
                                </select>
							</p>
							<p class="list">
								<label class="list">コメント</label> <input
									...
									value="<?php
        echo $targetTelop->chrComment;
        ?>" />
							</p>

							<p style="float: left; text-align: center; width: 300px; margin-top:40px;"
								id="buttonlist">
							...
							<div
								style="float: right; width: -100%; height: 100px; margin-top:40px; text-align: center; vertical-align: middle;">
...
echo '<table id="myTable" class="tablesorter search_table" style="border:0;padding:0;border-radius:5px;" >';
echo '<thead><tr>';
...
foreach ((array)$contents as $row) {
    ...
    Echo '<td style="width:52px;text-align:center;"><input type="radio" onclick="javascript: submit()" name="targetID" id="targetID" value="' . $row->chrShop_ID . '"/></td>';
    echo '<td style="width:70px;padding:2px;text-align: center;"><input class="center_button hvr-fade delete_button" style="width:70px; height:25px; margin:0;padding:0;font-weight:normal;" type="submit" name="' . $row->chrShop_ID . '" value="削除"/></td>';
    echo '</tr>';
}
echo '</tbody></table>';
    ...
    <!-- ********************  入力規則　開始      *********************** -->
                        <div id="sidr-right">
                            <?php
                                    ...
                                    $query = 'SELECT txtInstruction FROM form_helper WHERE chrPageName="telop";';
